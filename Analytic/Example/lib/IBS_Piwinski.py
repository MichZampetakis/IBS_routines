...
class PiwinskiIBS():
    ...
    # Integrate using interpolation - Quad
    def interp_integr(self, gx, gy, gs):
        int_gx = interp1d(self.Posi, gx)
        int_gy = interp1d(self.Posi, gy)
        int_gs = interp1d(self.Posi, gs)
        
        TTx = integrate.quad(int_gx, self.Posi[0], self.Posi[-1])[0] / self.Circu
        TTy = integrate.quad(int_gy, self.Posi[0], self.Posi[-1])[0] / self.Circu
        TTs = integrate.quad(int_gs, self.Posi[0], self.Posi[-1])[0] / self.Circu
        
        return TTx, TTy, TTs

    # Integrate using summation over elements' length
...
#         self.Tx, self.Ty, self.Ts = self.interp_integr(gx, gy, gs)
    ...
    # Evaluate the emittance evolution using Piwinski's model
    ...
    # Evaluate the emittance evolution using Piwinski's model, including Synchrotron Radiation!!!
    ...
    def emit_evol_with_SR(self, Emit_x, Emit_y, Sig_M, BunchL, EQemitX, EQemitY, EQsigmM, tau_x, tau_y, tau_s, dt):
        ...
        Evolemx = (- EQemitX + np.exp(dt * 2 * (float(self.Ixx) - 1.0 / tau_x)) 
                   * (EQemitX + Emit_x * (float(self.Ixx) * tau_x - 1.0))) / (float(self.Ixx) * tau_x - 1.0)
        Evolemy = (- EQemitY + np.exp(dt * 2 * (float(self.Iyy) - 1.0 / tau_y)) 
                   * (EQemitY + Emit_y * (float(self.Iyy) * tau_y - 1.0))) / (float(self.Iyy) * tau_y - 1.0)
        EvolsiM = np.sqrt((- EQsigmM**2 + np.exp(dt * 2 * (float(self.Ipp) - 1.0 / tau_s)) 
                           * (EQsigmM**2 + Sig_M**2 * (float(self.Ipp) * tau_s - 1.0))) / (float(self.Ipp) * tau_s - 1.0))
