...
print "-------------------------------------------------------"
print 'Np = {1:1.1e}       ||  Z  = {0:2.0f}'.format(Z,Nb)
print "-------------------------------------------------------"
...
turns_wanted = 16000
steps_update = 100
turns = turns_wanted / steps_update 
dt = steps_update / frev

''' # If you want time instead of turns
time_wanted = 1
time_update = 0.001
dt = time_update
'''

IBS_step = 1 #Growth rates are evaluated every IBS_step * steps_update turns if enabled

for i in range(0, turns_wanted, steps_update):
#for i in np.arange(0, time_wanted, time_update):
    print 'Turn = {}'.format(i)
