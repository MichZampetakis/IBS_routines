...
class NagaitsevIBS():
    ...
    def CoulogConst(self, Emit_x, Emit_y, Sig_M):
        ...
        sigtcm = 100 * self.Circu / 2. / np.sqrt(np.pi)
        ...
        Ncon   = self.Npart * self.c_rad**2 * c / (6 * np.sqrt(np.pi) * self.betar**3 * self.gammar**5 * self.Circu)
        return Ncon * coulog
    ...
    def Nagaitsev_Integrals(self, Emit_x, Emit_y, Sig_M):
        const = self.CoulogConst(Emit_x, Emit_y, Sig_M)
        ...
        Ip = integrate.simps(Ipi, self.posit) * const / Sig_M**2 * 2.

        return Ix, Iy, Ip


    def calculate_integrals(self, Emit_x, Emit_y, Sig_M):
        self.Ixx, self.Iyy, self.Ipp = self.Nagaitsev_Integrals(Emit_x, Emit_y, Sig_M)
        #print self.Ixx, self.Iyy, self.Ipp


    # Run if you want to evaluate the emittance evolution using Nagaitsev's Integrals.
    def emit_evol(self, Emit_x, Emit_y, Sig_M, dt):
    ...
    def emit_evol_with_SR(self, Emit_x, Emit_y, Sig_M, EQemitX, EQemitY, EQsigmM, tau_x, tau_y, tau_s, dt):
        #Ixx, Iyy, Ipp = self.Nagaitsev_Integrals(Emit_x, Emit_y, Sig_M, BunchL)
    ...
    def emit_evol_kicks(self, part_coord_dict):
        ...
        Ixx, Iyy, Ipp = self.Nagaitsev_Integrals(Emit_x, Emit_y, Sig_M)
...
class PiwinskiIBS():
    ...
    def Aconst(self, Ex, Ey, Sig_M):
        return self.c_rad**2 * c * self.Npart / (64 * np.pi**2 * self.gammar**4 * self.betar**3 * Ex * Ey * Sig_M * self.Circu / 2. / np.sqrt(np.pi))
